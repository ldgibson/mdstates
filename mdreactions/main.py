...
class Network:
    """
    Class used to analyze the different states of an MD trajectory.
    ...
    def _compute_distances(self, rep_id):
        ...
        ------
        AssertionError
            If `rep_id` is not a key in `replica`.
        AssertionError
            If the trajectory associated with `rep_id` does not exist.
        AssertionError
            If the atom pairs have not been determined.
        """
        assert rep_id in self.replica.keys(),\
            "`rep_id` is not a key in `replica`."
